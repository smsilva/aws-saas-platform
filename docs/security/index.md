...
|---|---|---|---|---|
| [SEC-002](issues/sec-002.md) | Medium | `04-install-alb-controller` | IAM policy downloaded from GitHub without SHA256 hash verification | Open |
| [SEC-003](issues/sec-003.md) | Low | `08-deploy-sample-app` | `kennethreitz/httpbin` image without fixed tag or digest | Open |
| [SEC-004](issues/sec-004.md) | Medium | `03-configure-access` | `AmazonEKSClusterAdminPolicy` scoped to entire cluster | Open |
| [SEC-005](issues/sec-005.md) | Low | `07-configure-alb-ingress` | ALB Security Groups created automatically, without source IP restriction | Open |
| [SEC-006](issues/sec-006.md) | Medium | `02-create-cluster` | IMDSv1 enabled by default on nodes — credentials accessible via SSRF | Open |
| [SEC-007](issues/sec-007.md) | Low | `09-configure-waf` | WAF without rate limiting — no protection against brute-force or flood | Resolved by script 15 |
